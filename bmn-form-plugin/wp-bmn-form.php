<?php
/*
Plugin Name: Brand Me Now Lead Form
Description: Lightweight version of the Brand Me Now wizard that focuses on the opening lead form while preserving CRM and agency integrations.
Version: 0.0.1
Author: Keeper
*/
...
class WP_BMN_Lead_Form_Plugin {
  const OPTION_KEY = 'wp_bmn_form_settings';
  ...
  // React Wizard Shortcode: loads Vite-built assets from brand-me-now-form/dist
  public function shortcode_bm_wizard($atts = [], $content = null){
    $dist_assets_dir = plugin_dir_path(__FILE__) . 'brand-me-now-form/dist/assets/';
    $dist_assets_url = plugins_url('brand-me-now-form/dist/assets/', __FILE__);
    ...
    if (!$js) {
      ...
      $expected_path = 'wp-content/plugins/' . $plugin_folder . '/brand-me-now-form/dist';
    ...
    echo '});\n';
    echo 'window.BMN_FORM_CONFIG = {';
    echo 'wpFormEndpoint: ' . json_encode(rest_url('agui-chat/v1/ghl/contact'));
    echo '};\n';
    echo '</script>';
...
new WP_BMN_Lead_Form_Plugin();

// Register REST API endpoints
add_action('rest_api_init', function(){ (new WP_BMN_Lead_Form_Plugin())->register_rest(); });
